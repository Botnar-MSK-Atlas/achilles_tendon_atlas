...
names(achilles.colours_0.2) <- c("Macrophages", "Vascular endothelial cells",  "Fast-twitch skeletal muscle cells",  "Slow-twitch skeletal muscle cells", "Adipocytes", "T cells", "Nervous system cells", "Lymphatic endothelial cells", "Mural cells", "NEGR1+ Fibroblasts", "ITGA10+ Fibroblasts", "Granulocytes", "Satellite cells", "B plasma cells",  "B cells")
...
Idents(so.achilles) <- so.achilles$cell_annotation_scVI_0.2
so.achilles
```
...
DimPlot(so.achilles, split.by = "microanatomical_site", ncol = 2, reduction = "umap.scvi")
ggsave(paste0(directory, "/Figures/UMAP_microanatomy.png"), width = 10, height = 8)
```
...
# add the scvi dim reduction
reducedDim(sce, "X_SCVI") <- Embeddings(so.achilles, "X_scVI") 
# add the umap reduction
reducedDim(sce, "UMAP.SCVI") <- Embeddings(so.achilles, "umap.scvi") 
...
#### Neighbourhood graph on scVI UMAP projection 
...
p1 <- plotNhoodGraphDA(achilles_milo, layout="UMAP.SCVI", milo_res=da_results, alpha=0.1)+
...
p2 <- plotNhoodGraph(achilles_milo, layout="UMAP.SCVI", colour_by=ini$cell_labels) +    
    scale_fill_manual(values=achilles.colours_0.2) +
...
umap_pl <- plotReducedDim(achilles_milo, dimred = "UMAP.SCVI", 
