...
```
```{r}
table(so.merge$orig.ident == "MSK1284-Ach-Enth")
```
Plot the number of cells per sample after the additional filtering

```{r}
df <- so.merge[[]] %>% select(orig.ident)
...
ggsave(paste0(directory, "/Integration_Figures.dir/Cluster_fraction_microanatomy.png"), 
       width = 8, height = 6, bg = "white")
```
Sample MSK1284 is completely filtered out if the soupX threshold filter is used. 
...
#so.merge
```
## Calculate and remove highly ambient genes

```{r, fig.width=16, fig.height=14}
# calculate highest contributing genes
subtraction <- so.merge@assays$RNA@counts - so.merge@assays$soupX@counts
...
#subtraction_fraction <- sweep(subtraction, 2, colSums(so.merge@assays$RNA@counts), '/')
...
p_RNA <- DotPlot(so.merge, 
                 features = rev(names(highest_contributors_ambiencereads))) + 
    scale_colour_viridis(direction = -1) +
    ggtitle("RNA assay") +
    coord_flip()

DefaultAssay(so.merge) <- "soupX"
p_so.mergeupX <- DotPlot(so.merge, 
                   features = rev(names(highest_contributors_ambiencereads))) + 
    scale_colour_viridis(direction = -1) +
    ggtitle("soupX assay") + 
    coord_flip()

p <- plot_grid(p_RNA, p_so.mergeupX, ncol = 2)
title <- ggdraw() + draw_label(so.merge@project.name, fontface='bold', size = 16)
p <- plot_grid(title, p, ncol=1, rel_heights=c(0.05, 1))
p
ggsave(paste0(directory, "/Integration_Figures.dir/Highest_ambient_genes.png"), p, 
...
ggsave(paste0(directory, "/Integration_Figures.dir/SoupX_scores_per_gene.png"),  
       width = 16, height = 8, bg = "white")
```

### Optionally Remove highly ambient genes
```{r}

if (ini$remove_ambient_genes == TRUE){
    genes_to_keep <- rownames(df[df$contamination_fraction < ini$ambient_genes_threshold,])
    so.merge <- subset(so.merge,features=genes_to_keep)
    so.merge
}

# which genes were removed?
print("The following highly ambient genes were removed")
rownames(df[df$contamination_fraction > ini$ambient_genes_threshold,])
