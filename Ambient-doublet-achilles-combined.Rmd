...
```{r, fig.width=14, fig.height=24, warning=FALSE, message=FALSE}
...
ggsave(paste0(directory, "/QC_figures/Mito_umap.", ini$file_type), 
    device = ini$file_type, width = 12, height = 24, bg = "white")
...
for (i in 1:length(sce_filter_manual)){
    plot_list[[i]] <- plotReducedDim(sce_filter_manual[[i]], dimred = dim_reduction, 
        ...
        scale_color_continuous(type = "viridis", limits = c(0,1), name = "decontX score")+
        ggtitle(mainExpName(sce_filter_manual[[i]]))
}
...
#### Plot the relationship between ambient RNA score and total counts

```{r, fig.width=14, fig.height=24}
plot_umi_vs_decontX <- function (sce_list){
    plot_list <- list()
    for (i in 1:length(sce_list)) {
      
        plot_list[[i]] <- colData(sce_list[[i]]) %>% 
            as_tibble() %>%
            ggplot(aes(x=sum, y=decontX_contamination )) + 
            geom_point(aes(colour = detected )) + 
            scale_colour_viridis()+
            geom_vline(xintercept = ini$sum_filter) +
            geom_hline(yintercept = ini$ambient_threshold)+
            scale_x_log10()+
            labs(x = "log(nUMIs)")+
    ...
    title <- ggdraw() + draw_label("UMIs vs % decontX contamination", fontface='bold', size = 16)
    p <- plot_grid(plotlist = plot_list, label_size = 12, ncol = 3)
...
p <- plot_umi_vs_decontX(sce_filter_manual)
p

```

```{r}
ggsave(paste0(directory, "/QC_figures/nUMI_vs_decontX.", ini$file_type), 
    device = ini$file_type, width = 12, height = 24, bg = "white")
```
...
NB in the following section, plotting functions are always defined.  
...
```{r, fig.width=14, fig.height=24}
if (ini$ambient_filter == TRUE){
p <- plot_umi_vs_decontX(sce_filter_ambient)
p
}
...
```{r}
if (ini$ambient_filter == TRUE){
ggsave(paste0(directory, "/QC_figures/Ambient_filter_nUMI_vs_decontX.", ini$file_type), 
    device = ini$file_type, width = 14, height = 24, bg = "white")
}
```
