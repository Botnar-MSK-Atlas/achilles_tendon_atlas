...
set_resolution <- function(so, resolution){
    so[["seurat_clusters"]] <- so[[paste0("leiden_X_scVI_snn_res.", resolution)]]
    Idents(so) <- so$seurat_clusters
    print(resolution)
    print(levels(Idents(so)))
    return(so)
}
# so.test <- set_resolution(so.fibroblasts, 0.25)
...
# enricher.results <- run_enricher(genes_counts, 0.25)
```
...
# visualise_enricher(enricher.results.2, 0.4, so.test, "Findmarkers_enricher")
```
...
run_pseudobulk_enricher(so.fibroblasts, 0.25)
...
get_markers <- function(resolution){
    ...
    top100_markers <- markers %>% group_by(cluster) %>% 
        ...
        slice_head(n = 200)
    return(top100_markers)

}
# top_markers <- get_markers(0.4)
...
run_enricher_2 <- function(markers, resolution){
    ...
    for(i in 1:length(unique(markers$cluster))){
        ...
        saveRDS(results[[i]], 
                paste0(directory, "/Findmarkers_enricher_", resolution, "/Cluster_", i, ".RDS"))    
...
# so.test <- set_resolution(so.fibroblasts, 0.4)
# enricher.results.2 <- run_enricher_2(top_markers, 0.4)
# visualise_enricher(enricher.results.2, 0.4, so.test, "Findmarkers_enricher")
```
...
run_findmarkers_enricher(so.fibroblasts, 0.25)
run_findmarkers_enricher(so.fibroblasts, 0.4)
```
...
gostres_out <- run_gost(0.4, "custom")
...
plot_gost <- function (results, resolution, background){
    ...
    ggplot(res, aes(x = query, y = term_name, size = query_size, colour = p_value)) +
                ...
                labs(colour = "-log10(p.adj)")+
                scale_y_discrete(position = "right")+
        ggtitle(paste0("Resolution ", resolution, ", ", background, " background"))
...
plot_gost(gostres_out, 0.4, "custom")
...
run_gost_findmarkers(0.25, "custom")
run_gost_findmarkers(0.25, "universe")
run_gost_findmarkers(0.4, "custom")
run_gost_findmarkers(0.4, "universe")
```
