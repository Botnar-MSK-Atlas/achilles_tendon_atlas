...
Simplify cell names by removing non-alphanumeric characters and spaces

```{r}
column <- paste0("cell_annotation_scVI_", ini$resolution)
so.achilles$cell_annotation_pseudobulk <- str_replace_all(so.achilles[[column]][, 1], " ", "") 
so.achilles$cell_annotation_pseudobulk <- str_replace(so.achilles$cell_annotation_pseudobulk, "\\+", "hi")
...
cell_plot <- DimPlot(so.achilles, reduction = 'umap.scvi', group.by = 'cell_annotation_pseudobulk', label = TRUE)
cond_plot <- DimPlot(so.achilles, reduction = 'umap.scvi', group.by = 'sample', shuffle = TRUE)
...
# filter samples: keep only those that express more than 1000 genes
for (i in 1:length(names(cts.split))){
    keep <- colSums(counts_list[[i]]) >= 1000
    counts_list[[i]] <- counts_list[[i]][, keep]
}
...
```
## Check if there are enough replicates to run DEseq2
Each microanatomical site should include cells from at least 3 donors
Each of these donors should express at least 1000 genes in that cell type (otherwise causes problems later)
...
# celltypes_list <- celltypes_list[c(1,3)]
```
...
save_DEseq2 <- function (DEseq2_results, celltype, ref_site){
    ...
    saveRDS(DEseq2_results, paste0(directory, "/DEseq2_results/", celltype, "_DEseq2_results.rds"))
...
save_clusters <- function (cluster_results, celltype){
    ...
    if (is.null(names(clusters)) == TRUE){ # skip cell types where no clustering was done
        ...
        saveRDS(clusters, 
                paste0(directory, "/Cluster_results/", celltype, "_cluster_results.rds"))
    }
...
DE_Enth <- build_summary(DEseq2_results_cells_Enth, "Enth")
DE_MTJ <- build_summary(DEseq2_results_cells_MTJ, "MTJ")
...
DE_Enth.long$count <- as.integer(DE_Enth.long$count)
# DE_Enth.long
DE_Enth.long$cell_annotation_comparison <- factor(DE_Enth.long$cell_annotation_comparison, 
                                 levels = c("Fibroblasts_MBvsEnth", "Fibroblasts_MTJvsEnth", "Fibroblasts_musclevsEnth", 
                                            ...
                                            "Skeletalmusclecells_MBvsEnth", "Skeletalmusclecells_MTJvsEnth", "Skeletalmusclecells_musclevsEnth",
                                            "Muralcells_MBvsEnth", "Muralcells_MTJvsEnth", "Muralcells_musclevsEnth",
                                            "Vascularendothelialcells_MBvsEnth", "Vascularendothelialcells_MTJvsEnth", "Vascularendothelialcells_musclevsEnth", 
                                            ...
                                            "Lymphaticendothelialcells_MBvsEnth", "Lymphaticendothelialcells_MTJvsEnth", "Lymphaticendothelialcells_musclevsEnth")
)
...
DE_MTJ.long$count <- as.integer(DE_MTJ.long$count)
# DE_Enth.long
DE_MTJ.long$cell_annotation_comparison <- factor(DE_MTJ.long$cell_annotation_comparison, 
                                 levels = c("Fibroblasts_EnthvsMTJ", "Fibroblasts_MBvsMTJ", "Fibroblasts_musclevsMTJ", 
                                            ...
                                            "Skeletalmusclecells_EnthvsMTJ", "Skeletalmusclecells_MBvsMTJ", "Skeletalmusclecells_musclevsMTJ",
                                            "Muralcells_EnthvsMTJ", "Muralcells_MBvsMTJ", "Muralcells_musclevsMTJ",
                                            "Vascularendothelialcells_EnthvsMTJ", "Vascularendothelialcells_MBvsMTJ", "Vascularendothelialcells_musclevsMTJ", 
                                            ...
                                            "Lymphaticendothelialcells_EnthvsMTJ", "Lymphaticendothelialcells_MBvsMTJ", "Lymphaticendothelialcells_musclevsMTJ")
)
```
