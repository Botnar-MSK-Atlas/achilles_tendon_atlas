...
Cluster 1, 2, 3, 6 are found across the tendon. 
Cluster 4 is MTJ/muscle specific. 
Cluster 5 is Enth specific
Cluster 7 is MTJ specific
...
> The other minor clusters may have more specific functions or may be artefacts. NB cluster 8 only has ~100 cells so might be hard to annotate. 
...
ggsave(paste0(directory, "/Marker_dotplots/Mouse_achilles.png"), width = 20, height = 20, bg = "white")
```
...
ggsave(paste0(directory, "/Marker_dotplots/Mouse_achilles.png"), width = 20, height = 20, bg = "white")
```
> Cluster 8 similar to pericytes
> Cluster 1/5 similar to "tendon fibroblasts 1"
> Cluster 2/3/4 similar to "tendon fibroblasts 2"
...
TC1 - no great match, maybe 3/4
TC2, no great match, maybe 2/3/4
TC3, kind of matches 2/3/4
TC4, matches cluster 1. "Attachment cells to the tendon-to-bone junction"
TC5, no great match, maybe 3/4
TC6, maybe cluster 4
TC7 no great match
TC8, kind of matches cluster 5. Pre-hypertrophic chondrocytes. Hoorah. 
TC9 matches cluster 5, hypertrophic chondrocytes. 
TC10 osteocytes?? 

Support cluster 5 as chondrocytes
...
> Cluster 8 looks like myonuclei (TTN)
...
>FibTen_Sub1 – similar to cluster 4 (MTJ/muscle-specific)
>FibTen_Sub 2/4 – similar to cluster 2/3
>FibTen_Sub 3/5 – similar to cluster 1/5/7
>Cluster 6 & 8 don’t match well. 
...
ggsave(paste0(directory, "/Annotation_figures/Hamstring_fibroblast_markers_2.png"), 
       device = "png", width = 10, height = 7, bg = "white")
```
> Clusters 2/3/4 like PDGFRA+ FB
> Cluster 1/7 overlap with the MKX+ markers  
...
plot_dotplot(so.fibroblasts, hamstring_nerve, "Hamstring nerve", resolution)
```
...
> Cluster 4 similar to Parenchymal FB. 
> Cluster 6 looks perineural/endoneural
...
plot_dotplot(so.fibroblasts, lai_broad, "Lai broad annotation", resolution)
```
...
plot_dotplot(so.fibroblasts, lai_myonuclei, "Lai myonuclei", resolution)
```
> Indicates there might be myonuclei in clusters 7 and 8
...
```
> Cluster 8 does indeed look like myonuclei, they are not particular to any of these subtypes perhaps because there are not many cells in cluster 8.
...
> Cluster 7 expresses COL3A1, COL1A1, COL1A2 which are in PI16 and and COL15A1 FB but they don't expressed the key named markers
> COMP is expressed in cluster 1 but no other markers
...
p
```
> Cluster 4 shares some markers with COL15A1+ fibroblasts(universally found/parenchymal). "These exhibited an association with the basement membrane, evidenced by expression of Col4a1, Hspg2 and Col15a1". These are all  expressed in cluster 4. HSPG2 is perlecan, and ECM protein invovled in adhesion, angiogenesis, basement membrane maintenance, neuromuscular junctions. 
...
> Looking like cluster 4 parenchymal, cluster 5 neurial and cluster 1/7 COMP+
...
Cluster 3 similar to c03(universal COL15A1 FB progenitors), found across most normal tissues tested (not synovium)
Cluster 1 and 3 similar to c05 (universal PI16 FB progenitors, found across most normal tissues inc synovium)
Clusters 1-4 similar to c09 (universal but more in inflamed/tumour)
Cluster 3 some markers of c15 (colon/stomach) 
Clusters 1-3 similar to c16 (universal SFRP2)
Cluster 3 and 6 very similar to c17 (ovary)
c10 synovium specific - PRG4 is specific to cluster1, but the rest of the genes are not expressed. 
...
plot_dotplot(so.fibroblasts, activation_markers, "Activation markers", resolution, fig_height = 12)
```

Not particularly enriched in a cluster. 

## 4. Sikkema et al 2023 
