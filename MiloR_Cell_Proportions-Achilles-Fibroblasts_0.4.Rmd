...
time <- format(Sys.time(), "%X") %>% str_replace_all(":", "-") %>%
    str_sub(1, 5)
directory <- paste0(date,"_", time, "_MiloR_Cell_Proportions-Fibroblasts.dir")
...
achilles_fibroblasts.colours <- c("#E25822",  # COMP+ adventitial tendon fibroblasts
                      "#BE0032",  # DCLK1+ adventitial tendon fibroblasts
                      "#023E52", # NEGR1hi_Muscle_Fibroblasts,
                          "#3986A0", # Myofibroblasts
                    "#75001F",    # Pericytes
                      "#FF9F79" # ACAN1hi_Chondrocytes, 
                   )  

names(achilles_fibroblasts.colours) <- c("COMP+ adventitial tendon fibroblasts", "DCLK1+ adventitial tendon fibroblasts", 
                                         "ITGA6+ nerve-associated fibroblasts ", "COL15A1+ universal fibroblasts", 
                                         "Skeletal muscle cells", "Chondrocytes")
...
```{r}
so.fibroblasts <- readRDS(paste0(ini$so_location, "/RDS_objects.dir/Achilles_fibroblast_subset.rds"))
so.fibroblasts
...
so.fibroblasts$cell_annotation_fibroblasts <- Idents(so.fibroblasts)
...
if (ini$subset_muscle == TRUE){
    so.fibroblasts <- subset(so.fibroblasts, subset = microanatomical_site != "muscle")
}
so.fibroblasts
...
DimPlot(so.fibroblasts, split.by = "microanatomical_site", ncol = 2, reduction = "umap")
...
reducedDim(sce, "X_SCVI") <- Embeddings(so.fibroblasts, "X_scVI") 
# add the umap reduction
reducedDim(sce, "UMAP") <- Embeddings(so.fibroblasts, "umap") 
...
#### Neighbourhood graph on UMAP projection 
...
p1 <- plotNhoodGraphDA(achilles_milo, layout="UMAP", milo_res=da_results, alpha=0.1)+
...
ggsave(paste0(directory, "/Figures/Neighbourhood_graph_DA.svg"), width = 7, height = 6)
```

Do the same plot but colour by original cluster

```{r, fig.width=9, fig.height=6}
p2 <- plotNhoodGraph(achilles_milo, layout="UMAP", colour_by="cell_annotation_fibroblasts") +   
...
ggsave(paste0(directory, "/Figures/Neighbourhood_graph_annotation.svg"), width = 9, height = 6)
...
ggsave(paste0(directory, "/Figures/Neighbourhood_graphs.svg"), width = 14, height = 5)
```
...
umap_pl <- plotReducedDim(achilles_milo, dimred = "UMAP", 
                          colour_by="microanatomical_site", 
                          text_by = "cell_annotation_fibroblasts", 
...
ggsave(paste0(directory, "/Figures/UMAP_vs_Neighbourhood_graph_DA.svg"), width = 10, height = 5, bg = "white")
```
...
ggsave(paste0(directory, "/Figures/Neighbourhood_homogeneity.svg"), bg = "white")
...
```{r}
da_results$cell_annotation_fibroblasts %>% unique()
da_results$cell_annotation_fibroblasts <- ifelse(da_results$cell_annotation_fibroblasts_fraction < 0.5, "Mixed", da_results$cell_annotation_fibroblasts)
```
...
a <- da_results %>% 
    ...
    filter(Diff == 1) %>%
    select(logFC, SpatialFDR, Diff, cell_annotation_fibroblasts) %>% 
    head(n=50)
print(a)
...
COMP+ adventitial tendon fibroblasts  
C6+C7+ tendon fibroblasts  
PRG4+ adventitial tendon fibroblasts
ITGA6+ nerve-associated fibroblasts  
COL15A1+ universal fibroblasts    
THBS4+ MTJ-specific fibroblasts
...
da_results$cell_annotation_fibroblasts <- factor(da_results$cell_annotation_fibroblasts, 
                                               levels = c("Mixed", 
                                                          "Chondrocytes", 
                                                          "COMP+ adventitial tendon fibroblasts",
                                                          "C6+C7+ tendon fibroblasts", 
...
ggsave(paste0(directory, "/Figures/Beeswarm_plot.svg"), width = 10, height = 10, bg = "white")
```
#### Plot the beeswarm without the "mixed" cells

```{r, fig.height=6, fig.width=8}
da_results_subset <- da_results %>% filter(cell_annotation_fibroblasts != "Mixed")

# remove "Mixed" from the factor
# make the cell annotation column a factor

da_results_subset$cell_annotation_fibroblasts <- factor(da_results_subset$cell_annotation_fibroblasts, 
...
plotDAbeeswarm(da_results_subset, group.by = "cell_annotation_fibroblasts")+
    theme(axis.title.y=element_blank())+
    scale_colour_gradient2(low = "#00466B", mid = "#FFFFFF", high = "#A91400", midpoint = 0, name = "logFC")

ggsave(paste0(directory, "/Figures/Beeswarm_plot_subset.png"), width = 10, height = 10, bg = "white")
ggsave(paste0(directory, "/Figures/Beeswarm_plot_subset.svg"), width = 10, height = 10, bg = "white")
