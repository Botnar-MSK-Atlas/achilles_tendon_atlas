...
dir_list <- list.dirs(ini$data_dir, recursive = FALSE)
...
data_dir <- paste0(wdir, ini$data_dir, ini$data_folder)
...
```{r}
cropped <- Crop(seurat[[ini$microanatomy]], x = c(2000, 3000), 
                y = c(0, 1000),
                coords = "plot")
seurat[["ROI1"]] <- cropped
seurat
...
```{r}
ImageFeaturePlot(seurat, "ACTA2", fov="ROI1", boundaries="centroids" , border.color = "black") + scale_fill_viridis_c()
...
```{r}
ImageFeaturePlot(seurat, "ACTA2", fov="ROI1", boundaries="segmentation", molecules=c("ACTA2", "CD163"), mols.size = .5, border.color = "black" ) + scale_fill_viridis_c()
```
...
FeatureScatter(seurat, "ACTA2", "CD163", jitter=T)
ggsave(paste0(directory, "/QC_plots/ACTA2_CD163_jitter.png"))
FeaturePlot(seurat, c("ACTA2", "CD163"))
ggsave(paste0(directory, "/QC_plots/ACTA2_CD163_featureplots.png"))
```
