---
title: "Xenium fibroblast annotation"
author: "Carla Cohen"
date: "2025-06-26"
...
time <- format(Sys.time(), "%X") %>% str_replace_all(":", "-") %>%
    str_sub(1, 5)
directory <- paste0(wdir, date,"_", time, "_Fibroblast_annotation.dir")
...
df <- df %>% mutate(fine_annotation =
                  case_when(is.na(predicted.id_fb_fine) ~ predicted.id_integrated_broad))
...
df.fb <- df %>% filter(predicted.id_integrated_broad == "Fibroblasts") %>% 
...
df <- as.data.frame(table(df.fb$predicted.id_fb_fine))
colnames(df) <- c("Cell_type", "predicted.id_fb_fine")
df2 <- as.data.frame(table(df.fb$predicted.id_integrated_fine))
colnames(df2) <- c("Cell_type", "predicted.id_integrated_fine")
df <- left_join(df, df2) %>% arrange(desc(predicted.id_fb_fine))
df$Cell_type <- factor(df$Cell_type, levels = df$Cell_type)
df <- df %>% pivot_longer(2:3, names_to = "labels", values_to = "Frequency")
df

ggplot(df, aes(fill = labels, y = Frequency, x = Cell_type))+
...
p1 <- DimPlot(so.fb, group.by = "predicted.id_integrated_fine", 
        cols = fibroblast.colours.bright, reduction = "umap")

p2 <- DimPlot(so.fb, group.by = "predicted.id_fb_fine", 
        cols = fibroblast.colours.bright, reduction = "umap")
...
saveRDS(so.fb, paste0(directory, "/RDS_objects.dir/Achilles_fibroblast_subset.rds"))
