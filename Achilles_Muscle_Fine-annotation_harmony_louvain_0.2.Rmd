...
Preliminary analysis of the clustering and multiple resolutions was performed: 20241121_09-50_Subset_Muscle.dir"
...
This script is for Harmony louvain at res 0.2
...
```{r}
so.muscle <- readRDS("20241121_09-50_Subset_Muscle.dir/RDS_objects.dir/Achilles_Muscle_subset.rds")
```
...
```{r, message = FALSE}
markers <- read.table(paste0("20241121_09-50_Subset_Muscle.dir/Marker_lists/Markers_", clustering, "_soupX_snn_res.", resolution, ".txt"), 
                sep = "\t")
...
## Remove cluster 6 as soup

```{r}
so.muscle <- subset(so.muscle, idents = "6", invert = TRUE)
...
rm(so.muscle.subset)
resolution <- 0.2
so.muscle$soupX_snn_res.0.2_subset <- Idents(so.muscle)
```
...
> These markers are pretty much all present in cluster 3
...
```
> Not super helpful because MFI and II markers are in clusters 0-4. 
...
```
> Type I myonuclei: cluster 0/1
> Type II myonuclei: cluster 4 + 2?6?8?
...
> Type I myonuclei: cluster 1/2
> Type II myonuclei: cluster 4
> MTJ myonuclei: cluster 3
> NMJ myonuclei: cluster 7
...
```
> Cluster 2 matches with both DCLK1+ and SAA2+ subtypes
> Cluster 4 matches ENOX1+ subtype
> Cluster 7 matches ID1+ (II)
...
```
> Modeul 1, cluster 0/1/3 = type I
> Module 2&3 Type II cluster 4
> Module 9 MTJ cluster 3
> Module 11 DCLK1(II), SAA2+ cluster 2
> Module 12 NMJ cluster 7
...
Cluster 0/2 type I
Cluster 2 Type II DCLK1+
Cluster 3 transitional (not here)
Cluster 4 Type II ENOX1+
Cluster 7 NMJ
...
Platelet-derived growth factor receptor A (PDGFRα) is a tyrosine kinase receptor that is used as the canonical marker for FAP isolation
...
Cluster 6 is likely immune cells 
...
for (i in levels(Idents(so.muscle))){
    cluster_markers[[i]] <- markers_subset_0.2 %>% 
        group_by(cluster) %>% 
...
0 - slow-twitch 1
1 - slow-twitch 2
2 - fast-twitch (DCLK1+ subtype)
3 - transitional/MTJ
4 - fast-twitch (ENOX1+ subtype)
5 - satellite cells
6 - immune
7 - NMJ
...
Clusters 0 and 1 are very similar. Find markers that distinguish these two clusters specifically. 

```{r}
markers_0v1 <- FindMarkers(so.muscle, assay = "soupX", ident.1 = "0", ident.2 = "1", 
                           only.pos = TRUE, min.pct = 0.25)
top_markers_0v1 <- markers_0v1 %>% 
        slice_max(n=20, order_by = abs(avg_log2FC)) %>% rownames()
top_markers_0v1
```
```{r}
FeaturePlot(so.muscle, top_markers_0v1[1:9], reduction = "umap", label = TRUE)
ggsave(paste0(directory, "/Feature_Plots/Cluster_0v1.png"), width = 10, height = 10, bg = "white")
DotPlot(so.muscle, 
             features = top_markers_0v1,
             assay = "soupX") + 
    scale_colour_viridis(direction = -1) +
    ggtitle("Cluster_0v1") + 
    theme(plot.title = element_text(size = 14, face = "bold"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+coord_flip()
    ggsave(paste0(directory, "/Top_20_Dotplots/Cluster_0v1.png"), width = 7, height =7, bg = "white")
```

```{r}
markers_1v0 <- FindMarkers(so.muscle, assay = "soupX", ident.1 = "1", ident.2 = "0", 
                           only.pos = TRUE, min.pct = 0.25)
top_markers_1v0 <- markers_1v0 %>% 
        slice_max(n=20, order_by = abs(avg_log2FC)) %>% rownames()
top_markers_1v0
```

```{r}
FeaturePlot(so.muscle, top_markers_1v0[1:9], reduction = "umap", label = TRUE)
ggsave(paste0(directory, "/Feature_Plots/Cluster_1v0.png"), width = 10, height = 10, bg = "white")
DotPlot(so.muscle, 
             features = top_markers_1v0,
             assay = "soupX") + 
    scale_colour_viridis(direction = -1) +
    ggtitle("Cluster_1v0") + 
    theme(plot.title = element_text(size = 14, face = "bold"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+coord_flip()
    ggsave(paste0(directory, "/Top_20_Dotplots/Cluster_1v0.png"), width = 7, height =7, bg = "white")
```

Many cluster 1 markers are shared with cluster 5 as well. 


Clusters 1 and 5 are similar. Find markers that distinguish these two clusters specifically. 

```{r}
markers_1v5 <- FindMarkers(so.muscle, assay = "soupX", ident.1 = "1", ident.2 = "5", 
                           only.pos = TRUE, min.pct = 0.25)
top_markers_1v5 <- markers_1v5 %>% 
        slice_max(n=20, order_by = abs(avg_log2FC)) %>% rownames()
top_markers_1v5
```
```{r}
FeaturePlot(so.muscle, top_markers_1v5[1:9], reduction = "umap", label = TRUE)
ggsave(paste0(directory, "/Feature_Plots/Cluster_1v5.png"), width = 10, height = 10, bg = "white")
DotPlot(so.muscle, 
             features = top_markers_1v5[1:20],
             assay = "soupX") + 
    scale_colour_viridis(direction = -1) +
    ggtitle("Cluster_1v5") + 
    theme(plot.title = element_text(size = 14, face = "bold"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+coord_flip()
    ggsave(paste0(directory, "/Top_20_Dotplots/Cluster_1v5.png"), width = 7, height =7, bg = "white")
```
These are all also present in cluster 0 and other clusters. 
...
features_10 <- markers_subset_0.2 %>% group_by(cluster) %>%
    dplyr::slice(1:10) %>%
...
features_5 <- markers_subset_0.2 %>% group_by(cluster) %>%
    dplyr::slice(1:5) %>%
    dplyr::select(cluster, gene)
...
features_20 <- markers_subset_0.2 %>% group_by(cluster) %>%
    dplyr::slice(1:20) %>%
    dplyr::select(cluster, gene)
...
ident_name <- "soupX_snn_res.0.2_subset"
...
Cluster 1 and 5 also have a lot of overlapping genes, which doesn't make so much sense. 
...
write.table(markers, 
            paste0(directory, "/Marker_lists/Markers_semi-annotated.txt"), 
                quote = FALSE, sep = "\t")
```
...
Use the sce syntax. Seurat version not working, probably as am using seurat v4 not 5. 
```{r}
so.muscle$semi_annotated <- Idents(so.muscle)
# convert to sce
sce <- as.SingleCellExperiment(so.muscle)
...
Which genes best label the fast twitch 1 and 2?

```{r}
markers_fast1v2 <- FindMarkers(so.muscle, assay = "soupX", ident.1 = "2 - Fast-twitch 1", ident.2 = "4 - Fast-twitch 2", 
                           only.pos = TRUE, min.pct = 0.25)
top_markers_fast1v2 <- markers_fast1v2 %>% 
        slice_max(n=20, order_by = abs(avg_log2FC)) %>% rownames()
top_markers_fast1v2
```
```{r}
FeaturePlot(so.muscle, top_markers_fast1v2[1:9], reduction = "umap", label = TRUE)
ggsave(paste0(directory, "/Feature_Plots/Fast1v2.png"), width = 10, height = 10, bg = "white")
DotPlot(so.muscle, 
             features = top_markers_fast1v2,
             assay = "soupX") + 
    scale_colour_viridis(direction = -1) +
    ggtitle("Cluster_Fast1v2") + 
    theme(plot.title = element_text(size = 14, face = "bold"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+coord_flip()
    ggsave(paste0(directory, "/Top_20_Dotplots/Fast1v2.png"), width = 7, height =7, bg = "white")
```

```{r}
markers_fast2v1 <- FindMarkers(so.muscle, assay = "soupX", ident.1 = "4 - Fast-twitch 2", ident.2 = "2 - Fast-twitch 1", 
                           only.pos = TRUE, min.pct = 0.25)
markers_fast2v1 <- markers_1v0 %>% 
        slice_max(n=20, order_by = abs(avg_log2FC)) %>% rownames()
markers_fast2v1
```

```{r, fig.width=7, fig.height=7}
FeaturePlot(so.muscle, markers_fast2v1[1:9], reduction = "umap", label = TRUE)
ggsave(paste0(directory, "/Feature_Plots/Fast_2v1.png"), width = 10, height = 10, bg = "white")
DotPlot(so.muscle, 
             features = markers_fast2v1,
             assay = "soupX") + 
    scale_colour_viridis(direction = -1) +
    ggtitle("Fast_2v1") + 
    theme(plot.title = element_text(size = 14, face = "bold"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+coord_flip()
    ggsave(paste0(directory, "/Top_20_Dotplots/Fast_2v1.png"), width = 7, height =7, bg = "white")
```

No better than from the other dotplot
...
literature_features <- c("TNNC1", "TNNI1", "TNNT1", "ATPA2","MYH7", 
                         ...
                         "NME7", "LHFPL3", "RASGRF1", "ASTN2", "PHYHIPL",
                         "CHRNA1", "COLQ", "PCDHG", "PHLDB2", "MUSK", "SCN5A")
...
unique_features <- c("NKAIN2", "CD36", "LRP1B", "BICC1", "ESRRG", #0
                      "UBC", "DES", "TCAP", "MYL2", "TNNC1", "CKM", "MB", #1
                      "COL19A1", "EFCAB7", "LRRK2", "RUNX1", "ARHGAP28", #2
                      "SORBS2","BICD1", "FHOD3", "XIRP2", "RBM20", #3
                      "ATP2A1", "TPM1", "MYBPC2", "TNNI2", "MYH1", #4
                      "PAX7", "CDH4", "CLCN5", "GPC6", "MARCHF1", "NAV2", "PLXDC2", #5
                      "MAN1A1", "NEGR1", "EBF1", "COL6A3", "SVEP1", "ELMO1", "SOX5",
 #6
                      "RASGRF1", "BRCA2", "KIAA1549L", "LHFPL3", "LIX1", "WDR17", "ITGAM", "SKAP1", #7
                      "PHLDB2", "MUSK", "SCN5A", "PLCB4", "PRKAR1A", "COL13A1", "CHRNA1", "TCF7L1", "CHRNE") #8


DotPlot(so.muscle, features = unique_features, assay = "soupX") + 
    scale_colour_gradient2(low = "#f7f3f2", mid = "#f2390a", high = "#6e1e0a", midpoint = 1)+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size = 16))+
    theme(axis.text.y = element_text(size = 16))+
    theme(axis.title.x = element_blank())+
...
0 - Slow-twitch
1 - Slow-twitch
2 - COL19A1hi Fast-twitch
3 - Transitional
4 - MYH1hi Fast-twitch
5 - Satellite cells
6 - Neuronal
7 - NMJ

```{r, fig.width=10, fig.height=7}
Idents(so.muscle) <- so.muscle$soupX_snn_res.0.2_subset
so.muscle <- RenameIdents(so.muscle, 
                          `0` = "Slow-twitch skeletal muscle cells",
                          `1` = "Slow-twitch skeletal muscle cells",
                          `2` = "COL19A1hi Fast-twitch skeletal muscle cells",
                          `3` = "Transitional skeletal muscle cells", 
                          `4` = "MYH1hi Fast-twitch skeletal muscle cells",
                          `5` = "Satellite cells",
                          `6` = "Neuronal cells",
                          `7` = "NMJ skeletal muscle cells")
so.muscle$fine_annotation_muscle <- Idents(so.muscle)

DimPlot(so.muscle, reduction = "umap", label = TRUE, repel = TRUE)
...
```{r, fig.width=10, fig.height=7}
DimPlot(so.muscle, reduction = "harmony.umap", label = TRUE, repel = TRUE)
ggsave(paste0(directory, "/Annotation_figures/UMAP_annotated_original_dims.png"), width = 6, height = 4, bg = "white")
```
```{r}
nerve_associated <- c("NTRK3", "TENM2", "KANK4", "SLC2A1", "SLC22A3", "CLDN1")
DotPlot(so.muscle, nerve_associated, assay = "soupX")
DotPlot(so.muscle, fibroblast_markers, assay = "soupX")
DotPlot(so.muscle, cellxgene_FAP, assay = "soupX")
```
