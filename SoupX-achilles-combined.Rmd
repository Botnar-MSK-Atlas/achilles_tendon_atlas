...
time <- format(Sys.time(), "%X") %>% str_replace_all(":", "-") %>%
    str_sub(1, 5)
directory <- paste0(date,"_", time, "_SoupX_Auto.dir")
...
#this example  uses a list of markers that are specifically expressed in adipocytemarkers
# NB Have tested other markers in script SoupX-manual-achilles-combined.Rmd
...
# update the gene lists to check ambient contamination post-correction
...
for(i in 1:length(so_soupX)){
    ggsave(paste0(directory, "/SoupX_figures/Dotplots/", 
                  so_soupX[[i]]@project.name, ".", ini$file_type), 
       plot = plot_list[[i]], device = ini$file_type, width = 16, height = 14, bg = "white")
}
...
Calculate most highly soupy genes  
Calculate soupX fraction per cell  
Plot dotplot and umap  
...
so_soupX <- lapply(so_soupX, contributing_genes)
```
