...
DimPlot(so, reduction = "harmony.umap", cols = achilles.colours_0.1, label = TRUE, shuffle = TRUE, repel = TRUE)+
    theme(legend.position="none")+
    xlab("UMAP_1")+
    ylab("UMAP_2")
...
Idents(so) <- so$cell_annotation_scVI_0.2
unique(Idents(so))
DimPlot(so, reduction = "harmony.umap", cols = achilles.colours_0.2, label = TRUE, shuffle = TRUE, repel = TRUE)+
    theme(legend.position="none")+
    xlab("UMAP_1")+
    ylab("UMAP_2")
