...
sce %>%
    scDotPlot(features = features_20,
              ...
              annoWidth = 0.02)
ggsave(paste0(directory, "/Annotation_figures/Dotplot_with_dendogram_logcounts_20.png"), width = 8, height = 20, bg = "white")
...
sce %>%
    scDotPlot(features = features_20,
              ...
              annoColors = list(ident_name = pal_d3()(n_colours),
                                "Marker_20" = pal_d3()(n_colours)),
              annoWidth = 0.02)
ggsave(paste0(directory, "/Annotation_figures/Dotplot_with_dendogram_scaled_20.png"), width = 8, height = 20, bg = "white")
...
However not when merged with the other cell types in fine annotation script. 

Let's get the most highly expressed genes for that cell type,by using pseudobulk.

```{r}
# 1. counts matrix - sample level

#Aggregation of counts to sample level per cell type
cts <- AggregateExpression(so.stromal, 
                    group.by = c("X_scVI_snn_res.0.4"),
                    assays = 'soupX',
                    slot = "counts",
                    return.seurat = FALSE)
...
df <- counts_list[["9"]] %>% 
    as.data.frame() %>%
    #arrange(desc(`9`)) %>% 
    slice_max(order_by = `9`, n = 50)
df
rownames(df)
write.csv(t(df), paste0(directory, "/Marker_list_for_cluster_9.csv"), quote = FALSE)

```

Still get out a list of muscle cell types from cellxgene. 
...
```{r}
FeaturePlot(so.stromal, features = "COL4A1", reduction = "umap", label = TRUE)
```
