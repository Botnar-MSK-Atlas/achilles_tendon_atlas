...
file_list_sce <- list.files(paste0(ini$soupX_files, "_SoupX.dir/RDS_objects.dir/SingleCellExp/"), 
                                 pattern="_SingleCellExp.rds")

names(file_list_sce) <- str_replace(file_list_sce, "_filtered_soupX_SingleCellExp.rds", "")

file_list_so <- list.files(paste0(ini$soupX_files, "_SoupX.dir/RDS_objects.dir/Seurat/"), 
                                 pattern="SeuratObject.rds")

names(file_list_so) <- str_replace(file_list_so, "_filtered_soupX_SeuratObject.rds", "")

```


Read in the sce objects & so 

```{r, echo=FALSE}

sce <- list()
so <- list()
for (i in 1:length(file_list_sce)){
    
    sce[[i]] <- readRDS(paste0(ini$soupX_files, 
                               "_SoupX.dir/RDS_objects.dir/SingleCellExp/",
                                                file_list_sce[i]))
    so[[i]] <- readRDS(paste0(ini$soupX_files, 
