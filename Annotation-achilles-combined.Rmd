...
```{r}
if (ini$integration_method == "harmony" & ini$cluster_method == "louvain"){
    print("harmony, louvain")
    so <- readRDS(paste0(ini$integration_date, "_Integration.dir/RDS_objects.dir/Achilles_harmony_SeuratObject.rds"))
} else if (ini$integration_method == "scVI" & ini$cluster_method == "louvain"){
    print("scVI, louvain")
...
} else if (ini$integration_method == "harmony" & ini$cluster_method == "leiden"){
    print("harmony, leiden")
    so <- readRDS(paste0(ini$integration_date, "_Cluster_leiden.dir/RDS_objects.dir/Achilles_harmony_SeuratObject.rds"))
} else if (ini$integration_method == "scVI" & ini$cluster_method == "leiden"){
    print("scVI, leiden")
    so <- readRDS(paste0(ini$integration_date, "_Cluster_leiden.dir/RDS_objects.dir/Achilles_harmony_SeuratObject.rds"))
}
...
if (ini$integration_method == "harmony" & ini$cluster_method == "leiden"){
...
```{r}
p <- DimPlot(object = so, label = TRUE, reduction = integrated_umap_name, shuffle = TRUE)+
...
p <- DoHeatmap(subset(so, downsample = 500), # randomly select cells from each cluster, otherwise it's too big to plot
               features = unique(top10markers$gene),
               assay = ini$normalisation_assay) + 
        scale_fill_viridis()+
