...
This time within the fibroblast subpopulations. 
...
time <- format(Sys.time(), "%X") %>% str_replace_all(":", "-") %>%
    str_sub(1, 5)
directory <- paste0(date,"_", time, "_Pseudobulk_Fibroblasts.dir")
...
How many cells are present at each microanatomy?
```{r}

so.fibroblast[[]] %>% select(microanatomical_site, cell_annotation_pseudobulk) %>% table()
```

```{r}
mtx <- data.frame(so.fibroblast[[]] %>% select(microanatomical_site, cell_annotation_pseudobulk) %>% table())
ggplot(mtx, aes(cell_annotation_pseudobulk, y = Freq))+
    geom_bar(aes(fill = microanatomical_site), stat = "identity")+
    theme_classic()+
    coord_flip()+
...
if (ini$subset_muscle == TRUE){
    so.fibroblast <- subset(so.fibroblast, subset = microanatomical_site != "muscle")
}
so.fibroblast
...
cell_plot <- DimPlot(so.fibroblast, reduction = 'umap', group.by = 'cell_annotation_pseudobulk', label = TRUE)
cond_plot <- DimPlot(so.fibroblast, reduction = 'umap', group.by = 'sample', shuffle = TRUE)
...
cts <- AggregateExpression(so.fibroblast, 
...
colData <- data.frame(sample = unique(so.fibroblast[["sample"]]))
colData <- left_join(colData, so.fibroblast[[]], keep = FALSE, multiple = "first") %>% 
...
find_celltypes_to_run <- function(celltype){
    ...
    print(paste("Enth", n_enth, sep = ": "))
    print(paste("MB", n_mb, sep = ": "))
    print(paste("MTJ", n_mtj, sep = ": "))
    print(paste("Muscle", n_muscle, sep = ": "))
...
# celltypes_list <- celltypes_list[c(1,2)]
```
...
build_summary <- function (DEseq2_results, ref_site){
    ...
    colnames(DEGs.df) <- c("cell_annotation_pseudobulk", "comparison", "Upregulated DEGs", "Downregulated DEGs", "Total DEGs")
...
DE_Enth.long <- DE_Enth.long %>% mutate (cell_annotation_pseudobulk_comparison = paste(cell_annotation_pseudobulk, comparison, sep = "_"))
...
ggplot(DE_Enth.long, aes(x = cell_annotation_pseudobulk_comparison, y = count))+
...
# merge column for cell annotation & comparison
DE_MTJ.long <- DE_MTJ.long %>% mutate (cell_annotation_pseudobulk_comparison = paste(cell_annotation_pseudobulk, comparison, sep = "_"))
DE_MTJ.long$count <- as.integer(DE_MTJ.long$count)

```

```{r, fig.width=12, fig.height=6}
ggplot(DE_MTJ.long, aes(x = cell_annotation_pseudobulk_comparison, y = count))+
    geom_point(aes(colour = comparison, shape = type), size = 3)+
    theme_bw()+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+
