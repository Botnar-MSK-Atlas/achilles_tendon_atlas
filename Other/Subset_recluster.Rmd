...
if (ini$integration_method == "harmony"){
    ...
    print("Number of dimensions in scVI reduction")
    print(ncol(scVI_embeddings))
    ...
    so.subset <- FindNeighbors(so.subset, reduction = "X_scVI", 
                               dims = 1:ncol(scVI_embeddings), # use all the dims that were calc by scVI
                               graph.name = c("X_scVI_nn", "X_scVI_snn"))

    # recalculate the UMAP using scVI embedidngs
    so.subset <- RunUMAP(so.subset, reduction = "X_scVI", dims = 1:ncol(scVI_embeddings))
}
...
if (ini$integration_method == "harmony"){
    ...
    if (ini$cluster_method == "louvain"){
        so.subset <- FindClusters(so.subset, resolution = seq(ini$cluster_start, ini$cluster_end, ini$cluster_by))
    } else if (ini$cluster_method == "leiden"){
        so.subset <- so.subset %>% FindClusters(resolution = seq(ini$cluster_start, ini$cluster_end, ini$cluster_by), 
                                              algorithm = "leiden",
...
```{r, fig.width=12, fig.height=10, warning=FALSE, message = FALSE, eval = FALSE}
