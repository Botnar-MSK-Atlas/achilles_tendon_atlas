...
```{r}
so.fibroblasts <- readRDS("20241108_10-07_Subset_fibroblasts.dir/RDS_objects.dir/Achilles_fibroblast_subset.rds")
```
...
Cluster 0, 1, 4 are found across the tendon. 
Cluster 2 is MTJ/muscle specific. 
Cluster 3 is Enth specific
Cluster 5 is MTJ specific
Cluster 7 is in Enth/MB
...
```
> Clusters 1 and 2 are dominant. 
> The other minor clusters may have more specific functions or may be artefacts. NB cluster 7 only has ~100 cells so might be hard to annotate. 

### FindMarkers

Read in the markers calculated by the script "Fibroblasts_subset_recluster.Rmd" from 20241108_10-07_Subset_fibroblasts.dir
...
TC1 - no great match, maybe 2/4
TC2, no great match
TC3, kind of matches 0/2
TC4, matches cluster 1. "Attachment cells to the tendon-to-bone junction"
TC5, no great matc
TC6, maybe cluster 3
TC7 no great match
TC8, kind of matches cluster 3 & 7. Pre-hypertrophic chondrocytes. Hoorah. 
TC9 matches cluster 3, hypertrophic chondrocytes. 
TC10 osteocytes?? 

Support cluster 3 as chondrocytes
TC4 matches cluster 1
Other clusters don’t match. 
...
> Cluster 6 looks like myonuclei (TTN)
...
> Cluster 7 looks like it might have a small proportion of myonuclei in it. 
...
>FibTen_Sub 2/4 – similar to cluster 3/4
>FibTen_Sub 3/5 – similar to cluster 1/3/5
>Cluster 6 & 7 don’t match well. 

This is reassuring because the cluster 4 is the one that is specific to MTJ/muscle and is the most similar to the most abundant fibroblasts in the MTJ paper. 
...
The authors conclude that FibTen_Sub3 (similar to cluster 1/5/7) is likely to have a specialised function, reponsible for the maintenance of the tendon side of the MTJ. However, I see these throughout the length of the tendon. 
...
ggsave(paste0(directory, "/Annotation_figures/Hamstring_fibroblast_markers_2.png"), 
       device = "png", width = 10, height = 7, bg = "white")
```
> Clusters 2 like PDGFRA+ FB
> Cluster 1 overlap with the MKX+ markers ?
...
> Cluster 4 v similar to nerve cells  
...
> ADAM12+ rupture – not seen in Achilles (healthy)
> ABCA10+ similar to cluster 4 (nerve associated?)
> NR4A1+ not seen here…wrong resolution?
> Dividing cells & nervous system cells don’t match from top genes
...
```
> Cluster 4 maps to perineural/endoneural
> Cluster 2 parenchymal
> Cluster 5 "tenocyte"
...
```
> CLuster 6 is clearly skeletal muscle 
...
```
> Indicates there might be myonuclei in cluster 8
...
```
> Again confirms that cluster 8 are probably myonuclei but can't get any more detail than that.  Most similar to modules 1 (type I) and 3 (type II)
...
> Looking like cluster 2 parenchymal, cluster 3 neurial and cluster 1 COMP+
...
> These markers map most closely to cluster 2. 
...
```
> c10 maps to Cluster 7: PRG4+ synovial lining FB. Yes
> c17 maps to Cluster 5: STMN1+ proliferative FB. Seems unlikely.
...
```
> Most clusters express some level of typical fibroblast markers. except 3 (enth) and 6 (MTJ/muscle)
...
> Cluster 6 express some muscle markers but there are not enough cells to distinguish fast and slow twitch.  
...
> Some of the muscle genes are expressed throughout the MTJ/muscle regions
...
> The cellxgene adipose schwann cell markers match well for cluster 6 but not any of the other marker lists. 
...
1 – tenocyte (from muscle)/FBLNhi from quads paper
2 – parenchymal/universal or FAP/COL15A1 (Buechler)
3 - chondrocytes
4 – perineural/endoneural
5 – express some muscle markers, MTJ specific
6 – skeletal muscle
7 – PRG4+ synovial lining fibroblasts. 
...
Clusters 2 and 6 are similar. Find markers that distinguish these two clusters specifically. 

```{r}
markers_2v6 <- FindMarkers(so.fibroblasts, assay = "soupX", ident.1 = "2", ident.2 = "6", 
                           only.pos = TRUE, min.pct = 0.25)
top_markers_2v6 <- markers_2v6 %>% 
        slice_max(n=20, order_by = abs(avg_log2FC)) %>% rownames()
top_markers_2v6
```
```{r}
FeaturePlot(so.fibroblasts, top_markers_2v6[1:9], reduction = "umap", label = TRUE)
ggsave(paste0(directory, "/Feature_Plots/Cluster_2v6.png"), width = 10, height = 10, bg = "white")
DotPlot(so.fibroblasts, 
             features = top_markers_2v6[1:20],
             assay = "soupX") + 
    scale_colour_viridis(direction = -1) +
    ggtitle("Cluster_2v6") + 
    theme(plot.title = element_text(size = 14, face = "bold"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+coord_flip()
    ggsave(paste0(directory, "/Top_20_Dotplots/Cluster_2v6.png"), width = 7, height =7, bg = "white")
```
CFD, MIR3171HG
...
n_colours <- length(unique(so.fibroblasts$louvain_X_scVI_snn_res.0.3))

# plot the log counts
sce %>%
    scDotPlot(features = features_10,
              group = "louvain_X_scVI_snn_res.0.3",
              groupAnno = "louvain_X_scVI_snn_res.0.3",
              featureAnno = "Marker_10",
              groupLegends = FALSE,
              annoColors = list("louvain_X_scVI_snn_res.0.3" = pal_d3()(n_colours),
...
0 - VCAN, CACNB2m TEX41
1 - FBLN1, CILP, DPP4, CDH13, ANGPTL7, MMP3, CLU
2 - PDGFRA, FAP, COL15A1, PI16, SMOC2, ARHGAP24
3 - Loads inc PLOD2, COL5A1, COL11A1, GPC5, RUNX1, ACAN, COL2A1, GPC6, PMEPA1, WWP2, GALNT18
4 - SLC22A3, PLCB4, TENM2, ITGA6, ARHGEF28, CNTN4
5 - THBS4, COL3A1, COL1A2, NCAM1, HMCN1, PIEZo2, ASPN<, LEPR, NCAM1, HMCN1, MYL2
6 - TRDN, ACTN2, TTN, NEB, MYBPC1
7 - PRG4, SORBS2, CD44, GFPT2, CD55, SEMA3C, SEMA5A, CRTAC1, CLIC5, ITGB8
...
unique_features <- c("VCAN", "CACNB2",
                     "FBLN1", "CILP", "DPP4", "CDH13", "ANGPTL7", "MMP3", "CLU",
                     "PDGFRA", "FAP", "COL15A1", "PI16", "SMOC2", "ARHGAP24",
                     "PLOD2", "COL5A1", "COL11A1", "GPC5", "RUNX1", "ACAN", "COL2A1", "GPC6", "PMEPA1", "WWP2", "GALNT18",
                     "SLC22A3", "PLCB4", "TENM2", "ITGA6", "ARHGEF28", "CNTN4",
                     "THBS4", "COL3A1", "COL1A2", "NCAM1", "PIEZO2", "ASPN", "LEPR", "HMCN1", "MYL2",
                     "TRDN", "ACTN2", "TTN", "NEB", "MYBPC1",
                     "PRG4", "SORBS2", "CD44", "GFPT2", "CD55", "SEMA3C", "SEMA5A", "CRTAC1", "CLIC5", "ITGB8") 
...
unique_features_0 <- c("VCAN", "CACNB2")
unique_features_1 <- c("FBLN1", "CILP", "DPP4", "CDH13", "ANGPTL7", "MMP3", "CLU")
unique_features_2 <- c("PDGFRA", "FAP", "COL15A1", "PI16", "SMOC2", "ARHGAP24")
unique_features_3 <- c("PLOD2", "COL11A1", "GPC5", "ACAN", "COL2A1", "GPC6", "PMEPA1", "WWP2")
unique_features_4 <- c("SLC22A3", "PLCB4", "TENM2", "ITGA6", "ARHGEF28", "CNTN4")
unique_features_5 <- c("THBS4", "COL3A1", "COL1A2", "NCAM1", "PIEZO2", "ASPN", "LEPR", "HMCN1", "MYL2")
unique_features_6 <- c("TRDN", "ACTN2", "TTN", "NEB", "MYBPC1")
unique_features_7 <- c("PRG4", "SORBS2", "GFPT2", "CD55", "SEMA5A", "CRTAC1", "CLIC5", "ITGB8") 
...
FeaturePlot(so.fibroblasts, features = unique_features_7, reduction = "umap", label = TRUE, repel = TRUE)
ggsave(paste0(directory, "/Feature_Plots/Unique_cluster_7.png"), width = 12, height =12, bg = "white")
...
VlnPlot(so.fibroblasts, features = unique_features_7, assay = "soupX")
ggsave(paste0(directory, "/Vln_Plots/Unique_cluster_7.png"), width = 12, height =12, bg = "white")
...
0 - VCAN+, CACNB2+
1 - MMP3, ANGPTL7, CILP # also a bi of expression in cluster 0 for all these
2 - COL15A1, FAP, PDGFRA, SMOC2
3 - ACAN, COL2A1, PLOD2
4 - TENM2, ITGA6, PLCB4
...
summary_features <- c("VCAN", "CACNB2", "TEX41",
                      "MMP3", "ANGPTL7", "CILP",
                      "COL15A1", "PDGFRA", "SMOC2",
                      "ACAN", "COL2A1", "PLOD2",
                      "TENM2", "ITGA6", "PLCB4",
                      "THBS4", "NCAM1", "PIEZO2",
                      "TRDN", "ACTN2", "MYBPC1",
...
FeaturePlot(so.fibroblasts, features = c("NEGR1", "COMP"), reduction = "umap", label = TRUE)
ggsave(paste0(directory, "/Feature_Plots/NEGR1_COMP.png"), width = 10, height = 4, bg = "white")

VlnPlot(so.fibroblasts, features = c("NEGR1", "COMP"), assay = "soupX")
...
so.fibroblasts$cell_annotation_fibroblasts_0.3 <- Idents(so.fibroblasts)
