...
Create a list of top 20 DE genes for each cluster of mouse achilles data and do a dotplot

```{r, fig.width=20, fig.height=20, message = FALSE, warning = FALSE}
plot_list <- list()
mouse_achilles_top20 <- list()
for (i in unique(mouse_achilles$subpopulation)){
    ...
    if (i != "Nerve cells 2"){ # this cell type doesn't work as no genes are found in the so
        mouse_achilles_top20[[i]] <- mouse_achilles %>% 
        group_by(subpopulation) %>% 
        slice_max(n=20, order_by = desc(p_val_adj)) %>% 
        filter(subpopulation == i) %>%
        pull (gene) %>%
    # convert to capital letters
        toupper()
    plot_list[[i]] <- plot_dotplot(so.fibroblasts, mouse_achilles_top20[[i]], paste0(i, "_20"), resolution)
...
ggsave(paste0(directory, "/Marker_dotplots/De_Micheli_top20.png"), width = 20, height = 20, bg = "white")
```
...
Create a list of top 20 DE genes for each cluster from Fu et al and do a dotplot
...
for (i in unique(fu_tables3$cluster)){
    print(i)
    fu_top20[[i]] <- fu_tables3 %>% 
        group_by(cluster) %>% 
        slice_max(n=20, order_by = abs(avg_log2FC)) %>% 
        filter(cluster == i) %>%
        pull (gene)
    plot_list[[i]] <- plot_dotplot(so.fibroblasts, fu_top20[[i]], paste0(i, "_20"), resolution)
    
}
    
# Plot all dotplots together
plot_grid(plotlist = plot_list, ncol = 4)
ggsave(paste0(directory, "/Marker_dotplots/Fu_top20.png"), width = 20, height = 20, bg = "white")
```
...
> Cluster 2 is the "tenocyte" marker (COMP/MKX)
...
> Cluster 6 looks like it might have a small proportion of myonuclei in it. 
...
The authors conclude that FibTen_Sub3 (similar to cluster 2/4) is likely to have a specialised function, reponsible for the maintenance of the tendon side of the MTJ. However, I see these throughout the length of the tendon. 
...
ggsave(paste0(directory, "/Annotation_figures/Hamstring_fibroblast_markers_2.png"), 
       device = "png", width = 10, height = 7, bg = "white")
```
> Clusters 3 like PDGFRA+ FB
> Cluster 2 overlap with the MKX+ markers  
...
```
> Indicates there might be myonuclei in clusters 3 & 6
...
```
> Cluster 6 does indeed look like myonuclei, they are not particular to any of these subtypes perhaps because there are not many cells in cluster 6.
...
VlnPlot(so.fibroblasts, features = c("SPARC", "COL3A1", "FBLN1"), ncol = 2, pt.size = 0)
```
...
Create a list of top 20 DE genes for each cluster from Gao et al and do a dotplot
...
for (i in unique(gao_tables2$cluster)){
    print(i)
    gao_top20[[i]] <- gao_tables2 %>% 
        group_by(cluster) %>% 
        slice_max(n=20, order_by = abs(avg_log2FC)) %>% 
        filter(cluster == i) %>%
        pull (gene)
    plot_list[[i]] <- plot_dotplot(so.fibroblasts, gao_top20[[i]], paste0(i, "_20"), resolution)
    
}
    
# Plot all dotplots together
plot_grid(plotlist = plot_list, ncol = 4)
ggsave(paste0(directory, "/Marker_dotplots/Gao_top20.png"), width = 20, height = 20, bg = "white")
```
...
> No particularly obvious activation signature  
...
```
> Most clusters express some level of typical fibroblast markers. except 5 (enth) and 8 (MTJ/muscle)
...
Cluster 1 - tendon FBLN+ adventitial fibroblasts
Cluster 2 - tendon COMP+ adventitial fibroblasts, like tendon, muscle. COMP+ from pan-FB paper, Hamstring MKX, Auads FBLN1. 
...
> TEX41 marks cluster 1

```{r}
markers_3v1 <- FindMarkers(so.fibroblasts, assay = "soupX", ident.1 = "3", ident.2 = "1", 
                           only.pos = TRUE, min.pct = 0.25)
top_markers_3v1 <- markers_3v1 %>% 
        slice_max(n=20, order_by = abs(avg_log2FC)) %>% rownames()
top_markers_3v1
```
```{r}
FeaturePlot(so.fibroblasts, top_markers_3v1[1:9], reduction = "umap", label = TRUE)
ggsave(paste0(directory, "/Feature_Plots/Cluster_3v1.png"), width = 10, height = 10, bg = "white")
DotPlot(so.fibroblasts, 
             features = top_markers_3v1[1:9],
             assay = "soupX") + 
    scale_colour_viridis(direction = -1) +
    ggtitle("Cluster_3v1") + 
    theme(plot.title = element_text(size = 14, face = "bold"))+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+coord_flip()
    ggsave(paste0(directory, "/Top_9_Dotplots/Cluster_3v1.png"), width = 7, height =7, bg = "white")
```
> many more markers in 3 but absent from 1. So it's like cluster 3 has all cluster 1 markers but some additional ones. 

### Summary dotplot

```{r, fig.width=10, fig.height=6}
summary_features <- c("DCLK1", "KAZN", "SCARA5", "FBN1", "PLCB1", "TEX41", # cluster 1
                      "MMP3", "ANGPTL7", "COMP", "THBS4", "MYBL1", "CDH13", "FBLN1", "MKX", # 2
                      "LRRTM4", "GREB1L", "COL4A1", "COL4A2", "ARHGAP24", "ACTA1", "PDGFRA", "ABCA10", "COL15A1", # 3 etc
                      "GPC6", "FN1", "KCNQ5", "ACAN", "SDK2", "SLC25A27",
                      "SLC22A3", "PLCB4", "TENM2", "ITGA6",
                      "NEB", "TTN", "MYBPC1", "TRDN")
...
1 - DCLK1+ adventitial tendon fibroblast
...
so.fibroblasts <- RenameIdents(so.fibroblasts, 
                   `1` = "DCLK1+ adventitial tendon fibroblasts",
                    `2` = "COMP+ adventitial tendon fibroblasts",
                   `3` = "COL15A1+ universal fibroblasts", 
                   `4` = "Chondrocytes",
                   `5` = "ITGA6+ nerve-associated fibroblasts",
                   `6` = "Skeletal muscle cells")
